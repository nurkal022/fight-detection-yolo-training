...
Автономная программа с графическим интерфейсом на pygame для сбора датасета
...
# Получаем директорию скрипта для автономной работы
SCRIPT_DIR = Path(__file__).parent.absolute()

# Конфигурация - все пути относительно папки скрипта
DATASET_DIR = SCRIPT_DIR / 'dataset'
PROGRESS_FILE = SCRIPT_DIR / 'dataset_progress.json'
...
# Размеры окна (минимальные и начальные)
...
class DatasetCollectorGUI:
    def __init__(self):
        ...
        self.video_width = self.window_width - self.panel_width
        self.video_height = self.window_height
        self.update_layout()
    ...
    def update_layout(self):
        """Обновить размеры элементов при изменении размера окна"""
        self.panel_width = max(280, min(400, int(self.window_width * 0.25)))
        self.video_width = self.window_width - self.panel_width
        self.video_height = self.window_height
    ...
    def load_config(self):
        ...
        if not self.classes_file.exists():
            ...
            print(f"   Создайте файл {self.classes_file} с форматом:")
            print("""
{
  "classes": [
    {
...
""")
            ...
            while self.camera_running and self.camera:
                try:
                    ...
                    if ret and frame is not None:
                        ...
                        # Получаем размеры кадра и текущие размеры видео-области
                        ...
                        try:
                            ...
                            final_frame = pygame.Surface((target_width, target_height))
    ...
    def draw_panel(self):
        ...
        panel_rect = pygame.Rect(self.video_width, 0, self.panel_width, self.window_height)
        pygame.draw.rect(self.screen, COLOR_PANEL, panel_rect)
        
        y_offset = 20
        
        # Заголовок
        self.draw_text(self.screen, "Dataset Collector", (self.video_width + self.panel_width // 2, y_offset), 
        ...
        if class_names:
            ...
            self.draw_text(self.screen, f"Class: {current_class}", 
                          (self.video_width + 20, y_offset), self.font_medium, COLOR_PRIMARY)
            y_offset += 40
            
            # Описание
            if class_data.get('description'):
                desc_lines = self.wrap_text(class_data['description'], self.panel_width - 40, self.font_small)
                for line in desc_lines:
                    self.draw_text(self.screen, line, (self.video_width + 20, y_offset), 
            ...
            self.draw_text(self.screen, f"Progress: {collected}/{target}", 
                          (self.video_width + 20, y_offset), self.font_medium, COLOR_TEXT)
            y_offset += 40
            
            # Прогресс-бар
            bar_width = self.panel_width - 40
            bar_height = 30
            bar_x = self.video_width + 20
            bar_y = y_offset
            ...
            self.draw_text(self.screen, f"{progress_pct:.1f}%", 
                          (self.video_width + self.panel_width // 2, bar_y + bar_height // 2), 
            ...
            self.draw_text(self.screen, f"Mode: {mode_text}", 
                          (self.video_width + 20, y_offset), self.font_medium, mode_color)
            y_offset += 50
            ...
            if self.mode == 'auto':
                ...
                if time_left > 0:
                    self.draw_text(self.screen, f"Next: {time_left:.1f}s", 
                                  (self.video_width + 20, y_offset), self.font_small, COLOR_WARNING)
                    y_offset += 30
        
        y_offset += 20
        
        # Статистика по всем классам
        self.draw_text(self.screen, "All Classes:", (self.video_width + 20, y_offset), 
        ...
        # Динамическое количество отображаемых классов в зависимости от высоты окна
        available_height = self.window_height - y_offset - 250  # Оставляем место для кнопок
        max_classes_display = max(3, available_height // 25)
        ...
        for idx, (class_name, class_data) in enumerate(classes_list[:max_classes_display]):
            ...
            if idx == self.current_class_index:
                pygame.draw.rect(self.screen, (60, 60, 60), 
                               (self.video_width + 10, y_offset - 5, self.panel_width - 20, 25))
            ...
            self.draw_text(self.screen, text, (self.video_width + 20, y_offset), 
        ...
        if len(classes_list) > max_classes_display:
            ...
            self.draw_text(self.screen, f"... and {remaining} more", 
                          (self.video_width + 20, y_offset), self.font_small, (150, 150, 150))
            y_offset += 25
        ...
        self.draw_text(self.screen, "Controls:", (self.video_width + 20, y_offset), 
                      self.font_medium, COLOR_TEXT)
        y_offset += 40
        
        # Кнопка Capture
        capture_btn = self.draw_button(
            (self.video_width + 20, y_offset), 
        ...
        manual_btn = self.draw_button(
            (self.video_width + 30 + (self.panel_width - 50) // 2, y_offset), 
            ((self.panel_width - 50) // 2, 35),
            "Manual", 
    ...
    def draw_video(self):
        """Отрисовать видео"""
        video_rect = pygame.Rect(0, 0, self.video_width, self.video_height)
        pygame.draw.rect(self.screen, (0, 0, 0), video_rect)
        
        try:
            with self.frame_lock:
                if self.current_frame:
                    # Масштабируем кадр под текущий размер видео-области
                    scaled_frame = pygame.transform.scale(self.current_frame, (self.video_width, self.video_height))
        ...
        if self.capture_flash:
            if time.time() - self.flash_time < 0.2:
                overlay = pygame.Surface((self.video_width, self.video_height))
